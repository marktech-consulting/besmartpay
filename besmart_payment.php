...
<?php
	...
	$brand=$x->data[0]->brand; 
	    ...
	    ?>
<label for="cardid<?=$i?>" class="wc_card_class" > <input type="radio" name="search_type" id="cardid<?=$i?>" value="<?=$user_uniqe_id;?>"/>  &nbsp;<b><?=$brand?> ending in <?=$last4?> (expires <?=$exp_month?>/<?=$exp_year?>)</b></label>
<script>
...
$method  = $stripe->customers->retrieveSource(
$customer_id,
$card_id,
[]
);
...
'amount' =>  $get_rate,
'description' =>  $main_statement,
'currency' => 'usd',
'statement_descriptor' => $this->statement =  $this->get_option( 'statement' ),
'metadata' => [
'order_id' => $order_id,
],
